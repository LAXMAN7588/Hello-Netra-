...
class OCREngine(
    ...
    private fun run(srcMat: org.opencv.core.Mat): OCREngineResult {
        ...
        val cropDebugDir = File(context.cacheDir, "ocr_crops").apply { mkdirs() }
        Log.d("HelloNetraOCR", "OCR Run: image=${srcMat.cols()}x${srcMat.rows()}, detected ${boxes.size} boxes")
        ...
        while (i < sortedBoxes.size) {
            ...
            while (next < sortedBoxes.size && batchCrops.size < batchSize) {
                ...
                if (crop.rows() > 0 && crop.cols() > 0) {
                    try {
                        val cropFile = File(cropDebugDir, "crop_${next}_${crop.cols()}x${crop.rows()}.png")
                        Imgcodecs.imwrite(cropFile.absolutePath, crop)
                    } catch (t: Throwable) {
                        Log.w("HelloNetraOCR", "Failed to save debug crop: ${t.message}")
                    }
                    batchCrops.add(crop)
            ...
            try {
                if (batchCrops.isNotEmpty()) {
                    ...
                    for (j in batchResult.texts.indices) {
                        ...
                        val b = sortedBoxes[boxIdx]
                        val cropW = batchCrops[j].cols()
                        val cropH = batchCrops[j].rows()
                        Log.d(
                            "HelloNetraOCR",
                            "Line #$boxIdx: text='$text' | conf=$confidence | crop=${cropW}x${cropH} | recInputShape=${batchResult.inputShape} | box=[(${b.points[0].x},${b.points[0].y}), (${b.points[1].x},${b.points[1].y}), (${b.points[2].x},${b.points[2].y}), (${b.points[3].x},${b.points[3].y})]"
                        )
